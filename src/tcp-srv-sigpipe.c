/* tcp-srv-sigpipe
 Demos how SIGPIPE crashes programs.
 Example usage:
    tcp-srv-pipe 7777
  then
    tcp-send-fail localhost 7777
 This will isten on port 7777, then use tcp-send-fail to create a connection
 that will close unexpectedly with a RST, generating a SIGPIPE condition
 on this server, causing the program to exit.
 This is the same as 'tcp-srv-one' sample program, except for SIGPIPE not
 suppressed.
 */
#include <ctype.h>
#include <errno.h>
...
int main(int argc, char *argv[])
{
    ...
    /* DON'T Ignore the send() problem like in other sample programs */
    /* signal(SIGPIPE, SIG_IGN); */

    if (argc < 2 || 3 < argc) {
        fprintf(stderr, "[-] usage: tcp-srv-sigpipe <port> [address]\n");
        return -1;
    }
