/* time-wait
 Demostrates TIME-WAIT errors
 Example usage:
    time-wait www.google.com 80 7777
 Parameters:
...
int main(int argc, char *argv[])
{
    ...
    again:
        ...
        count = send(fd, http_request, strlen(http_request), 0);
        if (count < 0) {
        ...
        for (;;) {
            ...
            if (count < 0) {
                ...
                fprintf(stderr, "[+] connectioned closed\n");
                break;
            }
        }
        fprintf(stderr, "[+] time elapsed = %u seconds\n", (unsigned)(time(0) - now));
