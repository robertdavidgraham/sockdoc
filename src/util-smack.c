/****************************************************************************
          ...
          SMACK - an Aho-Corasick search engine
...
static void
smack_copy_matches(
    struct SmackMatches *row, const size_t *new_ids, unsigned new_count)
{
    size_t *total_ids;
...
static void
smack_add_symbols(
    struct SMACK *smack, const unsigned char *pattern, size_t pattern_length)
{
    size_t i;
...
make_copy_of_pattern(
    const unsigned char *pattern, size_t pattern_length, unsigned is_nocase)
{
 ...
 ****************************************************************************/
void
smack_add_pattern(struct SMACK *smack, const void *v_pattern,
    size_t pattern_length, size_t id, unsigned flags)
{
    ...
    if (pattern_length == 0)
        pattern_length = strlen(v_pattern);
...
static void
DEBUG_set_name(
    struct SMACK *smack, const void *pattern, size_t length, unsigned state)
{
...
swap_rows(struct SMACK *smack, unsigned row0, unsigned row1)
{
    ...
    memcpy(
        &smack->m_state_table[row0], &smack->m_state_table[row1], sizeof(swap));
...
smack_fixup_wildcards(struct SMACK *smack)
{
    ...
    for (i = 0; i < smack->m_pattern_count; i++) {
        ...
        for (j = 0; j < pat->pattern_length; j++) {
            ...
            while (offset < j)
                smack_search_next(
                    smack, &row, pat->pattern, &offset, (unsigned)j);
 ...
 ****************************************************************************/
static unsigned
handle_match(const struct SMACK *smack, unsigned index,
 ...
 ****************************************************************************/
unsigned
smack_search(const struct SMACK *smack, const void *v_px, unsigned length,
 ...
 *****************************************************************************/
static size_t
inner_match(const unsigned char *px, size_t length,
 ...
 *****************************************************************************/
static size_t
inner_match_shift7(const unsigned char *px, size_t length,
    const unsigned char *char_to_symbol, const transition_t *table,
 ...
 *****************************************************************************/
size_t
smack_search_next(const struct SMACK *smack, unsigned *current_state,
    const void *v_px, unsigned *offset, unsigned length)
{
    ...
    if (!current_matches) {
        ...
        switch (row_shift) {
        case 7:
            i += inner_match_shift7(
                px + i, length - i, char_to_symbol, table, &row, match_limit);
            break;
        default:
 ...
 ****************************************************************************/
size_t
smack_next_match(const struct SMACK *smack, unsigned *current_state)
{
...
/****************************************************************************
 ****************************************************************************/
size_t smack_search_done(const struct SMACK *smack, unsigned *current_state)
{
    //unsigned found_count = 0;
    transition_t *table = smack->table;
    unsigned row_shift = smack->row_shift;
 ...
 ****************************************************************************/
unsigned
smack_search_end(const struct SMACK *smack, FOUND_CALLBACK cb_found,
...
smack_selftest(void)
{
    ...
    const char *patterns[] = { "GET", "PUT", "POST", "OPTIONS", "HEAD",
        "DELETE", "TRACE", "CONNECT", "PROPFIND", "PROPPATCH", "MKCOL",
        "MKWORKSPACE", "MOVE", "LOCK", "UNLOCK", "VERSION-CONTROL", "REPORT",
        "CHECKOUT", "CHECKIN", "UNCHECKOUT", "COPY", "UPDATE", "LABEL",
        "BASELINE-CONTROL", "MERGE", "SEARCH", "ACL", "ORDERPATCH", "PATCH",
        "MKACTIVITY", 0 };
    unsigned i;
    const char *text
...
#ifdef SMACKSTANDALONE
int
main(void)
{
    int err = smack_selftest();

    if (err) {
        fprintf(stderr, "[-] smack: selftest failed\n");
        return 1;
    } else {
        fprintf(stderr, "[+] smack: sefltest succeeded\n");
    }
}
#endif
