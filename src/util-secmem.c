 ...
 */
static size_t my_get_pagesize(void) {
  long pagesize;
 ...
 */
static void *my_mmap_allocate(size_t size) {
...
/****************************************************************************
 ****************************************************************************/
void util_secmem_free(void *in_p) {
  char *p = (char *)in_p;
  size_t full_size;
  size_t page_size;
  ...
  util_wipe(p, full_size - 2 * page_size);
...
void util_wipe(volatile void *p, size_t size) {
  static void *(*volatile my_memset)(void *, int, size_t) = memset;

  /* Set some known value (canary) */
  if (size)
    *(char *)p = 'A';
  ...
  /* Make sure our canary was cleared */
  if (size && *(char *)p == 'A')
    abort();
}
